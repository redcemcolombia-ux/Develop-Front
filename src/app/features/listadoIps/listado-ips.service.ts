...
export class ListadoIpsService {
  private readonly http = inject(HttpClient);
  private readonly baseUrl = API_BASE_URL;

  consultarIps(): Observable<ApiResponse<ConsultarIpsResponse>> {
    const headers = this.buildAuthHeaders();
    const url = `${this.baseUrl}/api/ips/consultar`;
    return this.http
      .get<ApiResponse<ConsultarIpsResponse>>(url, { headers })
      .pipe(catchError((e) => this.handle<ConsultarIpsResponse>(e)));
  }
