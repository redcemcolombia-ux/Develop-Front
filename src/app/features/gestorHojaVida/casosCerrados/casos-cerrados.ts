...
@Component({
  ...
  templateUrl: './casos-cerrados.html',
  styles: [`
    .platform-card__header.casos-cerrados__header {
      background: linear-gradient(135deg, #0d6efd 0%, #0b5ed7 100%);
      color: #fff;
    }

    .casos-cerrados__export-button {
      background-color: #198754;
      border-color: #198754;
      color: #fff;
    }

    .casos-cerrados__export-button:hover,
    .casos-cerrados__export-button:focus {
...
export class CasosCerrados implements OnInit {
    ...
    this.hojaVidaService.consultarHojasVida().subscribe({
      next: (response) => {
        if (response.error === 0 || response.error === '0') {
          ...
          // Filtrar solo casos cerrados con estado "Apto" o "No Apto"
          this.hojasVidaExistentes = todasLasHojas.filter((hoja: any) => {
            return this.esCasoCerrado(hoja);
          });
          ...
          if (this.hojasVidaExistentes.length === 0) {
            Swal.fire({
              icon: 'info',
              title: 'Sin datos',
              text: 'No se encontraron casos cerrados'
            });
          }
        } else {
          this.isLoadingConsulta = false;
  ...
  private esCasoCerrado(hoja: any): boolean {
    ...
    // Debe tener los 3 PDFs y ESTADO_CIERRE debe ser "Apto" o "No Apto"
    return pdfsCount === 3 &&
           hoja.ESTADO_CIERRE &&
           (hoja.ESTADO_CIERRE === 'Apto' || hoja.ESTADO_CIERRE === 'No Apto');
  }
  ...
  filtrarHojasVida(): void {
    if (!this.searchTerm.trim()) {
      ...
      this.hojasVidaFiltradas = this.hojasVidaExistentes.filter(hoja =>
        ...
        hoja.ESTADO_CIERRE?.toLowerCase().includes(term)
      );
    }
    this.currentPage = 1;
  }
  ...
  verDetalleHoja(hoja: any): void {
    ...
    html += `<h5 class="mb-0 me-3">Detalle de Caso Cerrado</h5>`;
    ...
    html += '<div class="card-header bg-primary text-white"><strong>✓ Información de Cierre</strong></div>';
  ...
  exportarAExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Casos Cerrados');

    const fecha = new Date().toISOString().split('T')[0];
    const nombreArchivo = `Casos_Cerrados_${fecha}.xlsx`;
