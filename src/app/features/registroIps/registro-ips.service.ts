...
export interface ConsultarIpsResponse {
  ips: Ips[];
  total: number;
  mensaje?: string;
}

export interface RegistrarIpsPayload {
  NOMBRE_IPS: string;
  NIT: string;
  DIRECCION: string;
  TELEFONO: string;
...
export interface RegistrarIpsResponse {
  mensaje?: string;
}
...
export class RegistroIpsService {
  ...
  registrarIps(payload: RegistrarIpsPayload): Observable<ApiResponse<RegistrarIpsResponse>> {
    const headers = this.buildAuthHeaders();
    const url = `${this.baseUrl}/api/ips/crearIps`;
    return this.http
      .post<ApiResponse<RegistrarIpsResponse>>(url, payload, { headers })
      .pipe(catchError((e) => this.handle<RegistrarIpsResponse>(e)));
  }
